...
fn count_direct_neighbours(plane: &Plane, row: usize, col: usize) -> usize {
    iproduct!(-1..=1, -1..=1)
        ...
        .filter(|&(dr, dc)| {
            plane.get_seat(row as i32 + dr, col as i32 + dc) == &Seat::Occupied
        })
        .count()
}
...
impl<'a> Iterator for PlaneIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        iproduct!(0..self.plane.rows, 0..self.plane.cols)
            .for_each(|(row, col)| {
        ...
        if state.iter().zip(self.plane.seats.iter())
            .filter(|(r1, r2)|
                r1.eq(r2)
            ).count() == self.plane.rows {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn test_seat_debug() {
        assert_eq!(format!("{:?}{:?}{:?}", Seat::Vacant, Seat::NoSeat, Seat::Occupied),"L.#");
    }
}
