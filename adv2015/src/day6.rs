        ...
        match input {
            ...
            _ if input.contains(',') => {
                return {
                    ...
                    Token::Coord(res_x.unwrap_or(0usize), res_y.unwrap_or(0usize))
                }
            }
            _ => Token::EOF,
        }
...
impl SantaInterpreter {
    ...
    fn get_state(&self) -> usize {
        self.state.iter().fold(0usize, |acc1, line| {
            acc1 + line
                .iter()
                .fold(0usize, |acc2, &bulb| acc2 + if bulb { 1 } else { 0 })
        })
    }
}
...
fn get_token(input: &str, from: usize) -> NextToken {
    ...
    for (i, c) in chars.enumerate() {
        ...
        match c {
            ...
            _ if c.is_whitespace() && !word.is_empty() => match word.as_str() {
                "turn" => word.push(' '),
...
mod tests {
    ...
    impl cmp::PartialEq for Call {
        fn eq(&self, other: &Self) -> bool {
            match (self, other) {
                ...
                (Call::Call(op1, c11, c12), Call::Call(op2, c21, c22)) => {
                    op1 == op2 && c11 == c21 && c12 == c22
                }
                _ => false,
            }
        }
    ...
    fn test_partial_eq() {
        ...
        assert_eq!(
            Call::Call(Operation::TurnOff, Coord(1, 2), Coord(3, 4)),
            Call::Call(Operation::TurnOff, Coord(1, 2), Coord(3, 4))
        );
        assert_ne!(
            Call::Call(Operation::TurnOff, Coord(1, 2), Coord(3, 4)),
            Call::Call(Operation::TurnOff, Coord(3, 4), Coord(1, 2))
        );
        assert_ne!(
            Call::Call(Operation::TurnOff, Coord(1, 2), Coord(3, 4)),
            Call::EOF
    ...
    fn test_get_token_toggle() {
        ...
        assert_eq!(
            get_token(input, 0),
            NextToken(Token::Toggle, 6),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 6),
            NextToken(Token::Coord(461, 550), 14),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 14),
            NextToken(Token::Through, 22),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 23),
            NextToken(Token::Coord(564, 900), input.len()),
            "Unexpected Token"
        );
    }
    ...
    fn test_get_token_turn_off() {
        ...
        assert_eq!(
            get_token(input, 0),
            NextToken(Token::TurnOff, 8),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 8),
            NextToken(Token::Coord(812, 389), 16),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 16),
            NextToken(Token::Through, 24),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 24),
            NextToken(Token::Coord(865, 874), input.len()),
            "Unexpected Token"
        );
    }
    ...
    fn test_get_token_turn_on() {
        ...
        assert_eq!(
            get_token(input, 0),
            NextToken(Token::TurnOn, 7),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 8),
            NextToken(Token::Coord(599, 989), 15),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 15),
            NextToken(Token::Through, 23),
            "Unexpected Token"
        );
        assert_eq!(
            get_token(input, 23),
            NextToken(Token::Coord(806, 993), input.len()),
            "Unexpected Token"
        );
    }
    ...
    fn test_parser() {
        ...
        assert_eq!(
            parser.next_operation(),
            Call::Call(Operation::TurnOn, Coord(499, 989), Coord(806, 992),)
        );
    ...
    fn test_multiline_parser() {
        let mut parser = Parser::new(String::from(
            "turn on 59,99 through 806,99\r\nturn off 812,389 through 865,874",
        ));
        assert_eq!(
            parser.next_operation(),
            Call::Call(Operation::TurnOn, Coord(59, 99), Coord(806, 99),)
        );
        assert_eq!(
            parser.next_operation(),
            Call::Call(Operation::TurnOff, Coord(812, 389), Coord(865, 874),)
        );
    }
    ...
    fn test_santa_interpreter_all_on() {
        ...
        assert_eq!(
            ...
            "{} bulbs are showing us Christmas, but we see {} only",
            LIGHT_MAX_SIZE * LIGHT_MAX_SIZE,
            answer
        );
    }
    ...
    fn test_santa_better_interpreter_all_on() {
        ...
        assert_eq!(
            answer,
            LIGHT_MAX_SIZE * LIGHT_MAX_SIZE,
            "{} is Christmas brightness, but we have {} only",
            LIGHT_MAX_SIZE * LIGHT_MAX_SIZE,
            answer
        );
    }
    ...
    fn test_santa_better_interpreter_all_toggle() {
        ...
        assert_eq!(
            answer,
            2 * LIGHT_MAX_SIZE * LIGHT_MAX_SIZE,
            "{} is Christmas brightness, but we have {} only",
            2 * LIGHT_MAX_SIZE * LIGHT_MAX_SIZE,
            answer
        );
    ...
    fn test_display() {
        ...
        assert_eq!(
            Call::Call(Operation::TurnOn, Coord(1, 2), Coord(3, 4)).to_string(),
            "on((1,2); (3,4))"
        );
    ...
    fn test_debug() {
        ...
        assert_eq!(
            format!("{:?}", NextToken(Token::Toggle, 13)),
            "NextToken(Toggle, 13)"
        );
